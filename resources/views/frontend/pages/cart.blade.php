...
@section('title', __('page_title.cart'))
    ...
    <div class="shopping-cart section">
        <div class="container">
            <div class="row">
                ...
                <div class="col-lg-6">
                    <div class="cart-head-main">
                        <div class="cart-head-txt">
                            <p>My Cart <span class="prod_cart_count">({{ $cartCount }})</span> </p>
                        </div>
                    </div>
                </div>

                <div class="col-lg-6 cart_item_exist {{ $cartCount < 1 ? 'd-none' : '' }}">
                ...
                <div class="col-lg-6 col-md-12 cart_item_exist {{ $cartCount < 1 ? 'd-none' : '' }}">
                    <form id="cart-update-form" class="cart-main-wrapper" method="POST">
                        @csrf
                                    ...
                                    <div class="cart-product-detail-main">
                                        <div class="product-detail">
                                            <div class="product-detail-head">
                                                <h4><a href="{{ route('product-detail', $cart->product->slug) }}"
                                                        target="_blank">{{ $cart->product->title }}</a></h4>
                                            </div>
                                            <div class="product-prise-main">
                                                <div class="main-prise">
                                                    <p><span>Rs.{{ number_format($cart->price, 2) }}</span>
                                                    </p>
                                                </div>
                                                <div class="cut-prise">
                                                    <p><span class="money">Rs.{{ $cart->product->price }}</span>
                                                    </p>
                                                </div>
                                            </div>
                                            <div class="counter-delete-main">
                                                <div class="input-group prod-qty">
                                                    <div class="button minus">
                                                        <button type="button" class="btn btn-primary btn-number btn-qty" data-type="minus"
                                                            data-field="quant[{{ $key }}]">
                                                            <i class="ti-minus"></i>
                                                        </button>

                                                        <?php /*<button type="button" class="btn btn-primary btn-number btn-qty"
                                                            {{ 1 < $cart->quantity ? '' : 'disabled' }} data-type="minus"
                                                            data-field="quant[{{ $key }}]">
                                                            <i class="ti-minus"></i>
                                                        </button>*/ 
                                                        ?>
                                                    ...
                                                    <input type="text" name="quant[{{ $key }}]"
                                                        class="input-number input-qty"
                                                        data-id="{{ $cart->product->slug }}" data-min="1"
                                                        data-max="{{ $cart->product->stock }}"
                                                        value="{{ $cart->quantity }}">
                                                    <input type="hidden" name="qty_id[]" value="{{ $cart->id }}">
                                                    <div class="button plus">
                                                        <button type="button" class="btn btn-primary btn-number btn-qty"
                                                        ...
                                                        <?php /*<button type="button" class="btn btn-primary btn-number btn-qty"
                                                            data-type="plus" data-field="quant[{{ $key }}]"
                                                            {{ $cart->product->stock > $cart->quantity ? '' : 'disabled' }}>
                                                            <i class="ti-plus"></i>
                                                        </button>*/ ?>
                                                ...
                                                <div class="delete-cart">
                                                    <button data-id="{{ $cart->id }}" class="dltBtn"> <img
                                                            src="frontend/img/delete-cart.svg" alt=""> </button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="col-lg-6 col-md-12 cart_item_exist {{ $cartCount < 1 ? 'd-none' : '' }}">
                    ...
                    <div class="total-amount">
                        ...
                        <div class="right">
                            ...
                            <ul>
                                <li class="order_subtotal">
                                    <p> MRP Total</p>
                                    <span class="sub_total">Rs.{{ number_format($subTotalAmount, 2) }}</span>
                                </li>
                                ...
                                @php
                                    ...
                                    if (session()->has('points')) {
                                        $points = number_format(session('points'));
                                ...
                                <div class="product-point-main">
                                    <p>Product Points</p>
                                    <span class="prod_points">-Rs.{{ $points }}</span>
                                </div>
                                <li class="last" id="order_total_price">Total
                                    Amount<span class="prod_total_amount">Rs.{{ number_format($totalAmount, 2) }}</span>
                                </li>
                            </ul>
                        ...
                        <div class="left">
                            <div class="coupon">
                                <form id="points-form" action="{{ route('cart.point') }}" method="POST">
                                    @csrf
                                    <input type="number" id="points-input" min="1" max="{{ $user->total_points }}"
                                        name="point" placeholder="Enter points" value="{{ $points > 0 ? $points : '' }}"
                                        required {{ session()->has('points') ? 'readonly' : '' }}>

                                    <button id="btn-point-remove" type="button"
                                        class="btn btn-point danger-btn {{ session()->has('points') ? '' : 'd-none' }}"
                                        name="submit" value="2">Remove</button>

                                    <button id="btn-point-add" type="button"
                                        class="btn btn-point {{ session()->has('points') ? 'd-none' : '' }}" name="
                                        submit" value="1">Apply</button>

                                </form>
                                <div class="total-incentive-main">
                                    <p>My Total Incentive</p>
                                    <span>{{ number_format($user->total_points) }} Points</span>
                                </div>
                            </div>
                        </div>

                        <div class="button5">
                            <a href="{{ route('order.summery') }}" class="btn place-order">Place Order</a>
                            <a href="{{ route('home') }}" class="btn buy-more">Buy More</a>
                        </div>

                    </div>
                    <!--/ End Total Amount -->
                </div>
                <div class="col-lg-12 col-md-12  cart_item_no_exist {{ $cartCount > 0 ? 'd-none' : '' }}">
                    <div class="d-flex justify-content-center empty-cart-main">
                        <span>Your Cart is empty!</span>
                        <a class="btn" href="{{ route('home') }}">Add Items</a>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            // Points Validation
            var points = parseInt("{{ $user->total_points }}");
            $('#points-input').keyup(function() {
                var pointVal = parseInt(this.value);
                if (pointVal > points) {
                    this.value = points;
                } else if (pointVal < 1) {
                    this.value = 1;
                        ...
                        if (willDelete) {
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    ...
                                    btnDel.parents('.my-cart-main-section').remove();
                                    if (result.cart_total < 1) {
                                        $('.cart_item_exist').addClass('d-none');
                                        $('.cart_item_no_exist').removeClass('d-none');
                                    } else {
                                        $(".order_subtotal span.sub_total").html("Rs. " +
                                            result.sub_total);
                                        $("span.prod_points").html("-Rs. " + result.points);
                                        $("span.prod_total_amount").html("Rs. " + result
                                            .total_amount);
                                    }
                                    $("span.total-count").html(result.cart_total);
                                    $("span.prod_cart_count").html(
                                        `(${result.cart_total})`);
            ...
            $('.btn-point').click(function(e) {
                var btnVal = $(this).val();
                var pointVal = $('#points-input').val();
                $.ajax({
                    url: "{{ route('cart.point') }}",
                    method: 'post',
                    headers: {
                        "Accept": "application/json"
                    },
                    ...
                    success: function(response) {
                        ...
                        $("span.prod_cart_count").html(
                            `(${result.cart_total})`);
                        if (btnVal == 1) {
                            $('#btn-point-add').addClass('d-none');
                            $('#btn-point-remove').removeClass('d-none');
                            $('#points-input').attr('readonly', 'readonly');
                        } else {
                            $('#btn-point-add').removeClass('d-none');
                            $('#btn-point-remove').addClass('d-none');
                            $('#points-input').removeAttr('readonly');
                            $('#points-input').val('');
                        }
                        swal(response.message);
                    },
            ...
            $('.btn-qty').click(function() {
                var div = $(this).parents('.prod-qty');
                var quantity = div.find('input.input-qty').val();
                var slug = div.find('input.input-qty').data('id');
                $.ajax({
                    ...
                    error: function(request, status, error) {
                        div.find('input.input-qty').val(quantity - 1);
                        swal(request.responseText);
                    }
                });
            });
